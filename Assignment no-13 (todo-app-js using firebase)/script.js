import { getFirestore, db, collection, addDoc, getDocs, deleteDoc, doc, updateDoc } from "./firebase/config.js";


const todoInput = document.getElementById('todo-input')
const addTodoItem = document.getElementById('add-btn')
const todoListItems = document.getElementById('todo-list')

const todoContainer = document.querySelector('.todo-container')

// let todoListValue = [];


const todosCollection = collection(db, "todos");

async function addTodoToFirebase(todoText) {

    await addDoc(todosCollection, { text: todoText });
}

async function getTodosFromFirebase() {
    const snapshot = await getDocs(todosCollection);
    return snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
}

async function deleteTodoFromFirebase(id) {

    const todoRef = doc(db, "todos", id);

    await deleteDoc(todoRef);

}

async function updateTodoInFirebase(id, Text) {
    const todoRef = doc(db, "todos", id);
    await updateDoc(todoRef, { text: Text })

}



const showTodoList = async () => {

    todoListItems.innerHTML = "";

    let todoListValue = await getTodosFromFirebase();
    todoListValue.forEach(currentTodo => {
        const elementliCreate = document.createElement('li');
        elementliCreate.setAttribute("data-id", currentTodo.id);

        elementliCreate.innerHTML = currentTodo.text;
        const iconSpan = document.createElement('span')
        iconSpan.innerHTML = `<svg  id="edit_item"  xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#1f1f1f"><path d="M200-200h57l391-391-57-57-391 391v57Zm-80 80v-170l528-527q12-11 26.5-17t30.5-6q16 0 31 6t26 18l55 56q12 11 17.5 26t5.5 30q0 16-5.5 30.5T817-647L290-120H120Zm640-584-56-56 56 56Zm-141 85-28-29 57 57-29-28Z"/></svg>
     <svg id="delete_item"  xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#1f1f1f"><path d="M280-120q-33 0-56.5-23.5T200-200v-520h-40v-80h200v-40h240v40h200v80h-40v520q0 33-23.5 56.5T680-120H280Zm400-600H280v520h400v-520ZM360-280h80v-360h-80v360Zm160 0h80v-360h-80v360ZM280-720v520-520Z" /></svg>
     `


        elementliCreate.appendChild(iconSpan);
        todoListItems.appendChild(elementliCreate);

    });
}


const editTodoList = async (e) => {
    const editIcon = e.target.closest('svg#edit_item');
    if (!editIcon) return;

    const li = editIcon.closest('li');
    const id = li.getAttribute('data-id');
    const currentText = li.firstChild.textContent.trim();
    const editedText = prompt("Edit item:", currentText);

    if (!editedText?.trim()) return;


    await updateTodoInFirebase(id, editedText.trim());
    await showTodoList();



}

const removeTodoList = async (e) => {
    const deleteIcon = e.target.closest('svg#delete_item');
    if (!deleteIcon) return;

    const li = deleteIcon.closest('li');
    const id = li.getAttribute("data-id");
    await deleteTodoFromFirebase(id);
    await showTodoList();

}





const addTodoList = async () => {



    let newTodo = todoInput.value.trim();


    if (!newTodo) return alert("Input cannot be empty!");


    let todoDup = await getTodosFromFirebase();
    const isDuplicate = todoDup.some((cur) => newTodo.toLowerCase() === cur.text.toLowerCase())
    if (isDuplicate) {
        alert("This item is already exists!");
         todoInput.value= "";
        return;
    }

    await addTodoToFirebase(newTodo);
    todoInput.value = " ";
    await showTodoList();





}






showTodoList();

addTodoItem.addEventListener('click', () => {
    addTodoList();
})

// todoListItems.addEventListener('click', (e) => {
//     editTodoList(e);
// })

todoListItems.addEventListener('click', (e) => {
    editTodoList(e);
    removeTodoList(e);
})